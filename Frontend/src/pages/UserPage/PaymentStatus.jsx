...
const PaymentStatus = () => {
  ...
  useEffect(() => {
    const checkPayment = async () => {
      ...
      try {
        // Khi truy cập qua ngrok: trình duyệt chặn request tới localhost (Private Network Access).
        // Dùng /api (proxy Vite -> backend) thay vì VITE_API_URL để request đi qua same-origin.
        const apiBase =
          window.location.hostname === "localhost"
            ? import.meta.env.VITE_API_URL || "http://localhost:8080"
            : "/api";
        const res = await fetch(
          `${apiBase}/vnpay/payment-info${query}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-blue-50 text-center relative">
      ...
      <div className="text-xl text-gray-700 space-y-5">
        ...
        <div className="text-[15px]">Đội ngũ PhimNet</div>
      </div>
